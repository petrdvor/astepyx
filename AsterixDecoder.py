...
class AsterixDecoder:
    ...
    def decode(self, bytes):
        ...
        self.p += 1  # skip cat
        # length = int.from_bytes(data[self.p:self.p+2], byteorder="big", signed=False)
        ...
        while self.p<length:
            ...
            # print(self.uap)  # ['010','020'.., None, ] each FX = None

            # items2 = []
            # inc = 0
            # start = time.time()
            # for i in range(fspec_octets_num*8-1,0,-1):
            #     if (1 << i) & fspec_octets and self.uap[inc] is not None:
            #         items2.append(self.uap[inc])
            #     inc+=1
            # end = time.time()
            # print('n', end-start)
...
if __name__ == '__main__':
    ...
    # with open(r'asterix_cat62.yml') as file:
    # # # The FullLoader parameter handles the conversion from YAML
    # # # scalar values to Python the dictionary format
    #     astspec = yaml.load(file, Loader=yaml.SafeLoader)
    #     print(astspec)
    spec = AsterixSpecification('specs/ast_spec_cat062_ed1.20.txt')
    astspec = spec.specification
    dec = AsterixDecoder(astspec)
    # pprint(dec.decode(data), sort_dicts=False)
